...
(function() {
  ...
  async function requireOperativo() {
    ...
    if (profErr || !["OPERATIVO","ADMIN"].includes(prof?.role?.toUpperCase())) {
  ...
  function renderNotifs({ checks, repos, below }) {
    const items = [
      ...
      { label: "REPOSICIONES APROBADAS", count: repos },
